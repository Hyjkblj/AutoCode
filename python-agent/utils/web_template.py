...
class WebTemplateGenerator:
    ...
    def generate(self, prompt: str, target: str = "web") -> WebTemplateResult:
        ...
        if self.llm_client.is_configured():
            try:
                llm_files = self._generate_via_llm(prompt_text)
                return WebTemplateResult(files=llm_files, used_fallback=False, reason="llm_generated")
            except Exception as exc:  # noqa: BLE001
                fallback = _fallback_files(prompt_text)
                return WebTemplateResult(files=fallback, used_fallback=True, reason=f"llm_fallback:{exc}")

        fallback = _fallback_files(prompt_text)
        return WebTemplateResult(files=fallback, used_fallback=True, reason="llm_not_configured")

    def _generate_via_llm(self, prompt: str) -> dict[str, str]:
        system_prompt = (
...
body {
  ...
  background: linear-gradient(145deg, #f6f8ff 0%, #eef3ff 100%);
  color: #1e2a3a;
}
