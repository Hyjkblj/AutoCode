...
public class EventDeduplicationService {
    ...
    public EventDeduplicationService(
            ...
            @Value("${event-service.deduplication.ttl-hours:24}") int ttlHours) {
        this.redisTemplate = redisTemplate;
        this.dedupTtl = Duration.ofHours(ttlHours);
    }
    ...
    public void markProcessed(String eventId, long sequenceNumber) {
        ...
        try {
            ...
            redisTemplate.opsForValue().set(dedupKey, "processed", dedupTtl);
            
            // Store sequence number with TTL for duplicate ACK responses
            redisTemplate.opsForValue().set(seqKey, String.valueOf(sequenceNumber), dedupTtl);
